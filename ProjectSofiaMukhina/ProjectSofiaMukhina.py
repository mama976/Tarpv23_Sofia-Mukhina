from math import *
print("Ruudu karakteristikud")
a=float(input('Sisesta ruudu külje pikkus => '))
S=a**2
print("Ruudu pindala", round(S))
P=4*a
print("Ruudu ümbermõõt", round(P))
di=a*sqrt(2)
print("Ruudu diagonaal", round(di,2))
print()
print("Ristküliku karakteristikud")
b=float(input("Sisesta ristküliku 1. külje pikkus => ")
c=foat(input("Sisesta ristküliku 2. külje pikkus => ")
S=b*c
print("Ristküliku pindala", round(S)
P=2*(b+c)
print("Ristküliku ümbermõõt", round(P))
di=sqrt(b**2+c**2)
print("Ristküliku diagonaal", round(di,2))
print()
print("Ringi karakteristikud")
r=float(tinput("Sisesta ringi raadiusi pikkus => "))
d=2*r
print("Ringi läbimõõt"+ str(d))
S=pi()*r**2
print("Ringi pindala", round(S))
C=2pi()*r
print("Ringjoone pikkus", round(C)
